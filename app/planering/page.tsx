...
export default function PlannerPage() {
  ...
  const mapStyleConfig = useRef({
    ...
    sources: {
      ...
      // för kommersiell drift). Proxyn byter till LM öppna data-WMTS när LM_OPEN_TOPO_TOKEN sätts.
      lantmateriet: {
        type: 'raster' as const,
    ...
    layers: [
      ...
      // LM-baskartan är TYST: nedtonad topowebb + avmättnad/kontrast/opacitet så förarens egna
  ...
  const handleMapReady = useCallback((map: any) => {
    ...
    // === Fotspår-ikon för stig/led (symbol-placement: 'line') ===
    {
      ...
      fpImg.onload = () => {
        try {
          ...
          const mapImg = canvasToMapLibreImage(c);
    ...
    map.addLayer({ id: 'tma-warning-glow', type: 'line', source: 'tma-warning-source', paint: { 'line-color': '#ff453a', 'line-width': 24, 'line-opacity': 0.4, 'line-blur': 8 }, layout: { 'line-cap': 'round', 'line-join': 'round' } });
    ...
    setBaseVis('lm-layer', mapType === 'lantmateriet');
    ...
    // Generera ikon-bilder för varje symboltyp via delad helper.
    // markerIconDefs (severity-färgsystem) + iconSvgPaths + canvas-builder
    // ligger nu i lib/marker-icons.ts — exakt samma källa som Cesium 3D Körvy
    // (CesiumScene.tsx) använder för billboard-ikoner. ~120 rader inline-data
    // borttagen, historik finns i git.
    const loadIconPromises = markerIconDefs.map(async (def) => {
      const mapImg = await loadMarkerImageForMaplibre(def.id);
  ...
  useEffect(() => {
    ...
    const drawOverlay = () => {
      ...
      for (let i = 0; i < pts.length; i++) {
        ...
        ctx.beginPath();
        ctx.arc(vx, vy, isFirst ? 7 : 5, 0, 2 * Math.PI);
        ctx.fillStyle = '#fff';
        ctx.fill();
        ctx.strokeStyle = '#1d9e75';
    ...
    // Slut ringen: bygg polygon, identifiera högar, öppna panelen.
    // (Hög-kalkyl + panel oförändrade — bara flyttade hit från onUp så "Klar" kan anropa dem.)
    const finalizeSkotning = () => {
      const coords = [...skotningCoordsRef.current];
      if (coords.length < 3) return; // behöver minst 3 hörn — behåll ritläget
    ...
    skotningFinalizeRef.current = finalizeSkotning; // "Klar"-knappen i mode-bannern anropar denna

    const onDown = (e: MouseEvent | TouchEvent) => {
      if ('button' in e && e.button !== 0) return;
      const clientX = 'touches' in e ? e.touches[0].clientX : e.clientX;
      const clientY = 'touches' in e ? e.touches[0].clientY : e.clientY;
      downX = clientX; downY = clientY; lastX = clientX; lastY = clientY;
      moved = false;
      pressing = true;
    };
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      // Rörelse > 5px ⇒ det var ett drag (inte ett tryck) → ritar inte
      if (Math.abs(clientX - downX) > 5 || Math.abs(clientY - downY) > 5) moved = true;
    };
    ...
    return () => {
      ...
      if (map.touchZoomRotate) map.touchZoomRotate.enable();
      skotningFinalizeRef.current = null;
      // Cleanup canvas
      if (skotningCanvasRef.current && skotningCanvasRef.current.parentNode) {
        skotningCanvasRef.current.parentNode.removeChild(skotningCanvasRef.current);
        skotningCanvasRef.current = null;
    ...
    const icons: Record<string, any> = {
      ...
      'menu-brandrisk': (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none">
          ...
          <path d="M12 22C12 22 9 19 9 16C9 13 12 10 12 10C12 10 15 13 15 16C15 19 12 22 12 22Z" fill="#ff453a" opacity="0.6" />
        </svg>
      ),
      'menu-briefing': (
        <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke={color} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  const uppdateraMatlager = (coords: [number, number][], yta: boolean) => {
    const src = mapInstanceRef.current?.getSource('measure-source') as any;
    if (!src) return;
    const features: any[] = [];
    if (coords.length >= 2) {
      features.push({
        type: 'Feature', properties: {},
        geometry: yta && coords.length >= 3
          ? { type: 'Polygon', coordinates: [[...coords, coords[0]]] }
          : { type: 'LineString', coordinates: coords },
      });
    }
  ...
  const activeMode: { label: string; onExit: () => void; exitLabel: string; onDone?: () => void; doneLabel?: string } | null =
    isDrawMode ? {
    ...
    <div style={{
      ...
      {!briefingMode && activeMode && (
        <div style={{
          ...
          <span style={{ flex: 1, fontSize: '15px', fontWeight: '600', lineHeight: 1.3 }}>
            {activeMode.label}
          </span>
          {activeMode.onDone && (
            <button
              type="button"
              onClick={() => { activeMode.onDone?.(); }}
              aria-label={`${activeMode.doneLabel || 'Klar'} — slut ytan`}
              style={{
                minHeight: 40,
                padding: '8px 18px',
                borderRadius: 10,
      ...
      {korvyActive && korvyZoneAlert && (
        <div style={{
          ...
          <span className="material-symbols-outlined" aria-hidden="true" style={{ fontSize: '30px', color: korvyZoneAlert.color, flexShrink: 0 }}>
      ...
      {isForare && valtObjekt?.status === 'planerad'
        ...
        >
          <span className="material-symbols-outlined" aria-hidden="true" style={{ fontSize: '22px' }}>play_arrow</span>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span className="material-symbols-outlined" aria-hidden="true"
              style={{ fontSize: '44px', color: '#ff9f0a', marginBottom: 12, display: 'inline-block' }}>
              check_circle
            </span>
            <div style={{ fontSize: '17px', fontWeight: '700', color: '#fff', marginBottom: 8 }}>
              Avsluta {valtObjekt.namn}?
            </div>
            <div style={{ fontSize: '13px', color: '#a8a8ad', marginBottom: 24, lineHeight: 1.4 }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{
                  ...
                  fontSize: '15px', fontWeight: '600', cursor: avsluterObjekt ? 'not-allowed' : 'pointer',
                  fontFamily: 'inherit',
                }}
              >
                Avbryt
          ...
          {isTracking && (() => {
            ...
            <g>
              ...
              <circle
                ...
                fill={gpsAccuracy != null ? (gpsAccuracy < 5 ? '#30d158' : gpsAccuracy <= 10 ? '#f59e0b' : '#ff453a') : colors.blue}
                stroke="#fff"
          ...
          {measurePath.length > 2 && (() => {
            ...
            return (
              <>
                ...
                <circle cx={end.x} cy={end.y} r={14} fill="#30d158" stroke="#fff" strokeWidth={3} />
      ...
      {isTracking && gpsAccuracy != null && (
        <div style={{
          ...
          backdropFilter: 'blur(8px)',
          color: gpsAccuracy < 5 ? '#30d158' : gpsAccuracy <= 10 ? '#f59e0b' : '#ff453a',
          fontSize: '13px',
          fontWeight: '600',
      ...
      {antalPrompt && (() => {
        ...
        const rubrik = antalPrompt.type === 'highstump' ? 'Hur många högstubbar?' : 'Hur många träd?';
        const val = m?.antal ?? 1;
        const setVal = (n: number) => setMarkers(prev => prev.map(mm => String(mm.id) === antalPrompt.markerId ? { ...mm, antal: Math.max(1, n) } : mm));
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 400 }}
            onClick={() => setAntalPrompt(null)}>
            <div style={{ background: '#000', borderRadius: '24px', padding: '28px', width: '86%', maxWidth: '360px', border: '1px solid rgba(255,255,255,0.15)' }}
              onClick={ev => ev.stopPropagation()}>
              <div style={{ fontSize: '17px', fontWeight: 600, color: '#fff', textAlign: 'center', marginBottom: '20px' }}>{rubrik}</div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '20px', marginBottom: '24px' }}>
                <button type="button" className="btn-press" onClick={() => setVal(val - 1)}
                  style={{ width: '52px', height: '52px', borderRadius: '26px', border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.06)', color: '#fff', fontSize: '28px', lineHeight: 1, cursor: 'pointer' }}>−</button>
                <input type="number" inputMode="numeric" min={1} autoFocus
                  value={val}
                  onFocus={ev => ev.target.select()}
                  onChange={ev => { const n = parseInt(ev.target.value, 10); setVal(Number.isFinite(n) ? n : 1); }}
                  style={{ width: '110px', textAlign: 'center', padding: '14px 0', borderRadius: '14px', border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.06)', color: '#fff', fontSize: '32px', fontWeight: 700, fontFamily: 'inherit', outline: 'none' }} />
                <button type="button" className="btn-press" onClick={() => setVal(val + 1)}
                  style={{ width: '52px', height: '52px', borderRadius: '26px', border: '1px solid rgba(255,255,255,0.15)', background: 'rgba(255,255,255,0.06)', color: '#fff', fontSize: '28px', lineHeight: 1, cursor: 'pointer' }}>+</button>
              </div>
      ...
      {markerMenuOpen && (() => {
        ...
        return (
          ...
          >
            ...
            {/* Antal (miljöhänsyn) */}
            {(marker.type === 'eternitytree' || marker.type === 'naturecorner' || marker.type === 'highstump') && (
              <div style={{ fontSize: '15px', color: '#8e8e93', textAlign: 'center', marginBottom: '12px' }}>
                Antal: <span style={{ color: '#fff', fontWeight: 600 }}>{marker.antal ?? 1}</span> {marker.type === 'highstump' ? 'stubbar' : 'träd'}
              </div>
            )}
            ...
            {marker.type === 'landing' && marker.roadCheck && (() => {
              ...
              <div style={{
                ...
                {/* Loading */}
                {rc.status === 'loading' && (
                  <div style={{ textAlign: 'center', color: '#8e8e93', fontSize: '13px', padding: '14px' }}>
                    <div style={{
                ...
                {rc.status === 'error' && (
                  <div style={{ textAlign: 'center', padding: '14px' }}>
                    <div style={{ color: '#8e8e93', fontSize: '13px', marginBottom: '8px' }}>{rc.message}</div>
                    <button
                      onClick={() => {
                    ...
                    {isAllmanOrOklar && (
                      <div style={sectionStyle}>
                        ...
                        {rc.requiresSpecialPermit && (
                          <div style={{ fontSize: '13px', color: '#ff453a', marginBottom: '4px' }}>Särskilt tillstånd krävs</div>
                        )}
                        ...
                        {speed && speed <= 80 && !rc.generelltTillstandApplied && (
                          <div style={{ marginTop: '14px', fontSize: '13px', color: 'rgba(255,255,255,0.35)', lineHeight: '1.5' }}>
                    ...
                    {isAllmanOrOklar && rc.checklist && (
                      <details style={sectionStyle}>
                        <summary style={{ ...sectionHeadingStyle, cursor: 'pointer', listStyle: 'none', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0' }}>
                          ...
                          <span style={{ fontSize: '13px', fontWeight: '500', color: checkedCount === 11 ? '#30d158' : 'rgba(255,255,255,0.3)', letterSpacing: '0', textTransform: 'none' as const }}>{checkedCount}/11</span>
                        </summary>
              ...
              {marker.isLine && marker.lineType === 'boundary' && (() => {
                ...
                return (
                  <div style={{ marginTop: '20px', borderTop: '1px solid rgba(255,255,255,0.08)', paddingTop: '20px' }}>
                    <div style={{ fontSize: '13px', fontWeight: '700', color: '#fff', marginBottom: '12px' }}>📝 Om trakten</div>
                      ...
                      return (
                        <div style={{ padding: '10px 12px', borderRadius: '10px', background: 'rgba(48,209,88,0.06)', border: '1px solid rgba(48,209,88,0.15)' }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                            ...
                            {!pendingAudio && !isRecNew && <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.2)' }}>Tryck för att spela in</div>}
                          </div>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => {
                    ...
                    {notes.length === 0 && editingNoteId !== 'new' && (
                      <div style={{ fontSize: '13px', color: 'rgba(255,255,255,0.2)', marginTop: '6px', textAlign: 'center' }}>Inga anteckningar ännu</div>
                    )}
                  </div>
                );
              })()}
      ...
      {overlays.brandrisk && brandriskData && !briefingMode && (
        <div style={{
          ...
          <div style={{
            ...
            height: '10px',
            borderRadius: '50%',
            background: brandriskData.fwiindex <= 0 ? '#30d158'
      ...
      {/* === MÄTNINGS-INDIKATOR === */}
      {/* Ersatt av det samlade geo-mätverktyget (siffra + Rensa/Stäng) högre upp. */}
      {false && measureMode && !isMeasuring && (
        <div style={{
          position: 'absolute',
          ...
          {measurePath.length > 1 ? (
            ...
            <>
              <span style={{ fontSize: '13px', opacity: 0.6, padding: '0 12px' }}>Dra för att mäta</span>
            </>
          )}
          {/* Stäng */}
          <button
      ...
      {/* Ersatt av det samlade geo-mätverktyget högre upp. */}
      {false && measureAreaMode && !isMeasuring && (
        <div style={{
          position: 'absolute',
          bottom: menuHeight + 130,
          left: '50%',
          ...
          {measurePath.length > 2 ? (
            ...
            <span style={{ fontSize: '13px', opacity: 0.6, padding: '0 12px' }}>Dra för att mäta yta</span>
          )}
          {/* Stäng */}
          <button
            onClick={() => {
        ...
        <div style={{
          ...
          <div style={{
            ...
            <span style={{ 
              ...
              height: '8px', 
              borderRadius: '50%', 
              background: gpsPaused ? '#f59e0b' : '#30d158',
              animation: gpsPaused ? 'none' : 'pulse 1s infinite',
            }} />
            <span style={{ fontSize: '13px', opacity: 0.6 }}>
              {gpsPath.length} pkt
            </span>
            {gpsPaused && <span style={{ fontSize: '13px', color: '#f59e0b' }}>PAUS</span>}
          </div>
      ...
      {(isDrawMode || isZoneMode) && currentDrawCoords.length === 0 && (
        <div style={{
          ...
          <span style={{ fontSize: '13px', opacity: 0.6, padding: '0 12px' }}>Dra för att rita</span>
          <button
            onClick={cancelDrawing}
            style={{
              width: '44px',
      ...
      {(isArrowMode || selectedSymbol) && (
        <div style={{
          ...
          <span style={{ fontSize: '13px', opacity: 0.6, padding: '0 12px' }}>Tryck för att placera</span>
          <button
            onClick={cancelDrawing}
            style={{
              width: '44px',
                  ...
                  {symbolCategories.map((category) => (
                    ...
                    >
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>{category.symbols.length} symboler</div>
                      </div>
                    ...
                    {symbolCategories.find(c => c.name === subMenu)?.symbols.map(type => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span style={{ fontSize: '13px', opacity: 0.6, textAlign: 'center' }}>
                            {type.name}
                          </span>
                        </div>
                      );
                ...
                <div style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Gå längs linjen</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" style={{ opacity: 0.3 }}>
                      <path d="M9 6 L15 12 L9 18" />
                    </svg>
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Tryck för punkter</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" style={{ opacity: 0.3 }}>
                      <path d="M9 6 L15 12 L9 18" />
                    </svg>
                ...
                <div style={{
                  ...
                  {lineTypes.filter(t => !t.id.includes('sideRoad') && !t.id.includes('backRoad')).map(type => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: '13px', opacity: 0.8 }}>{type.name}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Punkt till punkt</div>
                    </div>
                  </div>
        ...
        setGpsAccuracy(accuracy);
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Se avstånd till närmaste väg</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" style={{ opacity: 0.3 }}>
                      <path d="M9 6 L15 12 L9 18" />
                    </svg>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Se alla snitslade vägar</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" style={{ opacity: 0.3 }}>
                      <path d="M9 6 L15 12 L9 18" />
                    </svg>
            ...
            {activeCategory === 'gallring' && subMenu === 'stickvag-settings' && (
              <div style={{ padding: '20px' }}>
                <div style={{ marginBottom: '24px' }}>
                  <div style={{ fontSize: '13px', opacity: 0.6, marginBottom: '8px' }}>Målavstånd</div>
                ...
                <div style={{ marginBottom: '24px' }}>
                  <div style={{ fontSize: '13px', opacity: 0.6, marginBottom: '8px' }}>Tolerans (±)</div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                    <button
                ...
                <div style={{
                  ...
                  <div style={{ fontSize: '13px', opacity: 0.7 }}>Godkänt avstånd:</div>
                  <div style={{ fontSize: '20px', fontWeight: '700', color: '#30d158' }}>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Rotera kartan</div>
                    </div>
                    <div style={{
                      width: '44px',
                      height: '26px',
                      borderRadius: '13px',
                      background: compassMode ? '#30d158' : 'rgba(255,255,255,0.1)',
                      padding: '2px',
                      transition: 'background 0.2s ease',
                    }}>
                      <div style={{
                ...
                <div style={{
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '14px' }}>
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Avlägg vid allmän väg ≤80 km/h</div>
                    </div>
                  </div>
                ...
                <div style={{
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>Visa/dölj element</div>
                    </div>
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="1.5" style={{ opacity: 0.3 }}>
                      <path d="M9 6 L15 12 L9 18" />
                    </svg>
                  ...
                  <div
                    onClick={() => {
                      if (mapType === 'lantmateriet') setMapType('satellite');
                      else if (mapType === 'satellite') setMapType('terrain');
                      else if (mapType === 'terrain') setMapType('osm');
                      else setMapType('lantmateriet');
                    }}
                    style={{
                      padding: '16px 20px',
                      display: 'flex',
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '13px', opacity: 0.5, marginTop: '2px' }}>{showMap ? 'På' : 'Av'}</div>
                    </div>
                    <div style={{
                      width: '44px',
                      height: '26px',
                      borderRadius: '13px',
                      background: showMap ? '#30d158' : 'rgba(255,255,255,0.1)',
                      padding: '2px',
                      transition: 'background 0.2s ease',
                    }}>
                      <div style={{
      ...
      {editingMarker && (
        ...
        >
          ...
          >
            ...
            {(editingMarker.type === 'eternitytree' || editingMarker.type === 'naturecorner' || editingMarker.type === 'highstump') && (
              <div style={{ marginBottom: '16px', padding: '16px', borderRadius: '16px', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
                <div style={{ fontSize: '13px', color: '#8e8e93', marginBottom: '12px', textAlign: 'center' }}>Antal {editingMarker.type === 'highstump' ? 'stubbar' : 'träd'} den här punkten representerar</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                    ...
                    {hasAudio && !isRecThis && (
                      <button
                        ...
                        style={{ width: '32px', height: '32px', borderRadius: '16px', border: 'none', background: 'rgba(239,68,68,0.1)', color: '#ff453a', fontSize: '13px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                      >🗑</button>
                    )}
                  </div>
                  {hasAudio && !isRecThis && (
            ...
            <div style={{
              ...
              >
                <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="#30d158" strokeWidth="2.5">
      ...
      {showAvslutaBekraftelse && (
        <div style={{
          ...
          <div style={{ fontSize: '13px', opacity: 0.5, textAlign: 'center', marginBottom: '32px', lineHeight: 1.5, color: '#fff' }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ opacity: 0.5, fontSize: '13px' }}>Sparade vägar</span>
              <span style={{ fontSize: '13px' }}>{markers.filter(m => m.isLine && ['sideRoadRed', 'sideRoadYellow', 'sideRoadBlue'].includes(m.lineType || '')).length} st</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
              <span style={{ opacity: 0.5, fontSize: '13px' }}>Symboler</span>
              <span style={{ fontSize: '13px' }}>{markers.filter(m => m.isMarker).length} st</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              <span style={{ opacity: 0.5, fontSize: '13px' }}>Zoner</span>
              <span style={{ fontSize: '13px' }}>{markers.filter(m => m.isZone).length} st</span>
            </div>
          </div>
        ...
        <div style={{
          ...
          <div style={{ position: 'absolute', top: '50px', left: '14px', zIndex: 110, pointerEvents: 'auto' }}>
            <div style={{
              ...
              <div style={{ width: '6px', height: '6px', background: gpsPaused ? '#666' : '#30d158', borderRadius: '50%' }} />
              <span style={{ fontSize: '13px', color: '#fff', opacity: 0.6 }}>GPS</span>
            </div>
          </div>
                ...
                <>
                  ...
                  <div style={{
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px' }}>
                      <span style={{ opacity: 0.5, fontSize: '13px', color: '#fff' }}>Längd</span>
                      <span style={{ fontSize: '13px', color: '#fff' }}>{Math.round(längd)} m</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ opacity: 0.5, fontSize: '13px', color: '#fff' }}>Kommentar</span>
                      <span style={{ fontSize: '13px', opacity: 0.5, color: '#fff' }}>
                ...
                <>
                  ...
                  {selectedOversiktItem.comment && (
                    <div style={{
                      ...
                      <div style={{ opacity: 0.5, fontSize: '13px', color: '#fff', marginBottom: '8px' }}>Kommentar</div>
                      <div style={{ fontSize: '13px', color: '#fff' }}>{selectedOversiktItem.comment}</div>
                    </div>
                  )}
        ...
        >
          ...
          >
            ...
            <div style={{ 
              ...
              <div style={{ 
                fontSize: '13px', 
                color: checklistItems.every(i => i.answer !== null) ? '#30d158' : '#fbbf24',
                fontWeight: '500',
              }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                background: checklistItems.every(i => i.answer !== null) ? '#30d158' : '#fbbf24',
            ...
            >
              ...
              {tmaWeather && tmaWeather.status === 'done' && (
                <div style={secStyle}>
                  ...
                  {/* Vädervarningar */}
                  {windWarningRed && (
                    <div style={{ marginTop: '10px', fontSize: '13px', fontWeight: '500', color: '#ff453a' }}>
              ...
              <details open style={secStyle}>
                <summary style={summaryStyle}>
                  ...
                  {answeredCount === 7 && (
                    <span style={{ fontSize: '13px', fontWeight: '500', letterSpacing: '0', textTransform: 'none' as const, color: riskLevel === 'low' ? '#30d158' : riskLevel === 'medium' ? '#eab308' : '#ff453a' }}>
                ...
                <div style={{ marginTop: '12px' }}>
                  ...
                  {/* Resultat */}
                  {riskLevel && (
                    <div style={{ marginTop: '8px', fontSize: '13px', fontWeight: '500', lineHeight: '1.5', color: riskLevel === 'low' ? '#30d158' : riskLevel === 'medium' ? '#eab308' : '#ff453a' }}>
              ...
              <details open={bRisk[3] === true} style={secStyle}>
                <summary style={summaryStyle}>
                  ...
                  {bSamrad.kvitterad ? (
                    <span style={{ fontSize: '13px', fontWeight: '500', letterSpacing: '0', textTransform: 'none' as const, color: '#30d158' }}>Genomfört</span>
                  ) : (
                    <span style={{ fontSize: '13px', color: 'rgba(255,255,255,0.2)' }}>›</span>
                  )}
                </summary>
                      ...
                      <div style={{ display: 'flex', gap: '12px', alignItems: 'flex-end' }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: '#30d158', color: '#000', fontSize: '13px', fontWeight: '600',
                            cursor: 'pointer',
                          }}
                        >
                          Samråd genomfört
      ...
      {tractAnalysisOpen && tractAnalysis[tractAnalysisOpen] && (() => {
        ...
        const typeConfig: Record<string, { icon: string; color: string; label: string; link?: string }> = {
          ...
          naturreservat: { icon: '🛑', color: '#30d158', label: 'Naturreservat', link: 'https://skyddadnatur.naturvardsverket.se/' },
          ...
          nyckelbiotop: { icon: '🌿', color: '#30d158', label: 'Nyckelbiotop', link: 'https://www.skogsstyrelsen.se/skogensparlor' },
          ...
          <div style={{
              ...
              <>
                ...
                {analysis.errors.length > 0 && (
                  <div style={{ marginTop: '12px', padding: '10px 14px', background: 'rgba(239,68,68,0.1)', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.2)' }}>
                    <div style={{ fontSize: '13px', color: 'rgba(239,68,68,0.8)' }}>
            ...
            {analysis.status === 'error' && (
              <div style={{
                ...
                <div style={{ fontSize: '32px', marginBottom: '8px' }}>❌</div>
                <div style={{ fontSize: '13px', color: 'rgba(239,68,68,0.8)' }}>Analysen misslyckades</div>
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                <button
                  ...
                  style={{
                    ...
                    background: 'rgba(255,255,255,0.06)',
                    color: 'rgba(232,240,224,0.7)', fontSize: '13px', fontWeight: '600', cursor: 'pointer',
                  }}
                >← Tillbaka</button>
                <button
                  onClick={() => {
                  ...
                  style={{
                    ...
                    background: 'rgba(48,209,88,0.12)',
                    color: '#30d158', fontSize: '13px', fontWeight: '600', cursor: 'pointer',
                  }}
                ...
                grouped.map((h, i) => (
                  ...
                  >
                    <div style={{
                      ...
                      background: multiSelectChecked[i] ? '#30d158' : 'transparent',
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      flexShrink: 0,
                    }}>
                    ...
                    <div style={{ fontSize: '13px', color: '#8e8e93', fontWeight: '500', flexShrink: 0 }}>
                      {h.volym.toFixed(1)} m³
                    </div>
                  </div>
                ))
                ...
                .map(([s, v], i) => {
                  ...
                  return (
                    <div key={i} style={{
                      ...
                      <span style={{ flex: 1, fontSize: '13px', color: '#fff' }}>{s}</span>
